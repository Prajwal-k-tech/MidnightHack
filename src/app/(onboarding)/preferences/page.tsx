...
// Define generic attributes and their operator types for the rule builder
const attributes = {
      ...
      <div className="space-y-6 text-left">
        ...
        <div className="p-4 border rounded-lg bg-muted/30">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-2 mt-2">
            <Select value={currentAttribute} onValueChange={v => { setCurrentAttribute(v); setCurrentOperator(''); }}>
          ...
          {(!rules || rules.length === 0) ? (
            ...
            rules.map(rule => (
              <div key={rule.id} className="flex items-center justify-between rounded-lg border p-3 bg-muted/20">
                ...
                <Button variant="ghost" size="icon" className="h-8 w-8 flex-shrink-0" onClick={() => removeRule(rule.id)}>
